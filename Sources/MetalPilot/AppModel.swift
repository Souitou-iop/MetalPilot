...
final class AppModel: ObservableObject {
    ...
    func launch() {
        ...
        didLaunch = true
        DiagnosticFileLogger.write("App launched, version 4.3.3")
        Task {
            do {
            ...
            if let hosts = try? String(contentsOfFile: "/etc/hosts", encoding: .utf8),
               hosts.contains(HostsFileEditor.markerStart) || hosts.contains(HostsFileEditor.legacyMarkerStart) {
    ...
    func launchSelectedHUDApps(_ paths: [String]) {
        ...
        runTask(tr("正在启动所选游戏并注入 Metal HUD…", "Launching selected games with Metal HUD…", "選択したゲームを起動中…")) {
            ...
            if launchedNames.isEmpty {
                throw MetalPilotError.commandFailed(message)
            }
            return message
        }
    }
    ...
    func terminateSingleInterferingProcess(_ process: MetalHUDProcess, force: Bool = false) {
        runTask(tr("正在关闭 \(process.name)…", "Closing \(process.name)…")) {
            ...
            if ok {
                ...
                throw MetalPilotError.commandFailed(tr("关闭 \(process.name) 失败", "Failed to close \(process.name)"))
            }
        }
    }
    ...
    func increaseCrossOverPriority() {
        runTask(tr("正在检测 CrossOver", "Detecting CrossOver")) {
            ...
            guard !processes.isEmpty else {
                throw MetalPilotError.commandFailed(tr("未检测到 CrossOver 或 Wine 进程", "No CrossOver or Wine process found"))
            }
    ...
    func increaseFavoriteProcessPriority() {
        ...
        runTask(tr("正在优化常用进程", "Optimizing favorite processes")) {
            ...
            guard !matches.isEmpty else {
                throw MetalPilotError.commandFailed(tr("未检测到收藏的常用进程", "No saved favorite process is running"))
            }
            guard matches.count <= ConfigurationStore.maxFavoriteProcessNames else {
                throw MetalPilotError.commandFailed(tr("匹配的常用进程超过 64 个，请编辑常用进程后重试", "More than 64 favorite processes matched; edit favorites and try again"))
            }
            self.status.phase = .awaitingAuthorization
        ...
        hoyoTask = Task {
            do {
                ...
                guard let priorityPIDs, !priorityPIDs.isEmpty else {
                    throw MetalPilotError.commandFailed(tr("\(waitSeconds) 秒后未检测到 Wine 进程", "No Wine process detected after \(waitSeconds) seconds"))
                }
    ...
    func mountSelectedDisks() {
        ...
        guard assignments.count == selectedDiskIDs.count, !assignments.isEmpty else {
            report(MetalPilotError.invalidPath(tr("请为每个磁盘选择路径", "Choose a path for every volume")))
            return
        }
        runTask(tr("正在挂载磁盘", "Mounting volumes")) {
            ...
            guard failures.isEmpty else { throw MetalPilotError.commandFailed(tr("挂载失败并已回滚：\(failures.joined(separator: ", "))", "Mount failed and rolled back: \(failures.joined(separator: ", "))")) }
        ...
        Task {
            do {
                ...
                guard !configuration.restorableDiskMounts.isEmpty else {
                    throw MetalPilotError.commandFailed(tr("没有可恢复的挂载记录", "No previous mounts to restore"))
                }
    ...
    func toggleSteamDeck() {
        runTask(tr("正在读取设备名称", "Reading hostnames")) {
            ...
            if current.computerName == "steamdeck" {
                guard let backup = self.configuration.hostnameBackup else { throw MetalPilotError.commandFailed(tr("找不到原始设备名称备份", "Hostname backup is missing")) }
    ...
    private func restoreMounts(from volumes: [DiskVolume], manual: Bool = false) async {
        ...
        guard !assignments.isEmpty else {
            if manual {
                report(MetalPilotError.commandFailed(tr("没有找到可恢复的磁盘和路径", "No matching volume and path found to restore")))
                return
            }
        ...
        if succeeded.count == assignments.count {
            ...
            report(MetalPilotError.commandFailed(tr("自动恢复上次挂载失败", "Failed to restore previous mounts")))
        }
    }
