    ...
    public func run(_ executable: String, arguments: [String]) async throws -> CommandResult {
        try await Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                throw MetalPilotError.commandFailed(error.localizedDescription)
            }
            ...
            guard result.exitCode == 0 else {
                throw MetalPilotError.commandFailed(result.errorString.isEmpty ? "Command failed (\(result.exitCode))" : result.errorString)
            }
            return result
        }.value
    }
...
public enum InputValidation {
    ...
    public static func normalizedAbsolutePath(_ value: String, home: String = NSHomeDirectory()) throws -> String {
        ...
        guard path.hasPrefix("/"), path != "/", !path.contains("\0") else {
            throw MetalPilotError.invalidPath(value)
        }
        return path
    }
