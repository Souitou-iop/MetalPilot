import Foundation
import Testing
@testable import MetalPilotCore
...
@Test func pathValidationNormalizesAndRejectsRoot() throws {
    ...
    #expect(throws: MetalPilotError.invalidPath("/")) { try InputValidation.normalizedAbsolutePath("/") }
...
@Test func hostsEditorMigratesLegacyRebrandBlock() {
    let legacy = "127.0.0.1 localhost\n# BEGIN MAC GAME TOOLBOX HOYO\n0.0.0.0 a.example\n# END MAC GAME TOOLBOX HOYO\n"
    let domains = ["a.example"]
    let migrated = HostsFileEditor.replacingManagedBlock(in: legacy, domains: domains, enabled: true)
    #expect(migrated.contains(HostsFileEditor.markerStart))
    #expect(!migrated.contains(HostsFileEditor.legacyMarkerStart))
    #expect(migrated.contains("127.0.0.1 localhost"))
    #expect(migrated.contains("0.0.0.0 a.example"))
    let disabled = HostsFileEditor.replacingManagedBlock(in: migrated, domains: domains, enabled: false)
    #expect(disabled == "127.0.0.1 localhost\n")
}
...
actor HostnameRunner: CommandRunning {
    func run(_ executable: String, arguments: [String]) async throws -> CommandResult {
        ...
        case "HostName": throw MetalPilotError.commandFailed("HostName: not set")
        default: throw MetalPilotError.commandFailed("unexpected fixture")
        }
    }
}
...
actor RejectingPrivilegedOperator: PrivilegedOperating {
    ...
    func perform(_ operation: PrivilegedOperation) async throws {
        ...
        throw MetalPilotError.authorizationCancelled
    }
}
...
@Test func cacheClearAuthorizesBeforeDeletingUserFiles() async throws {
    ...
    do {
        ...
        #expect(error as? MetalPilotError == .authorizationCancelled)
    }
...
actor MockRunner: CommandRunning {
    ...
    func run(_ executable: String, arguments: [String]) async throws -> CommandResult {
        ...
        if arguments.contains("mount"), let failingMount, arguments.contains(where: { $0.contains(failingMount) }) {
            throw MetalPilotError.commandFailed("fixture failure")
        }
...
@Test func mountAcceptsEquivalentCanonicalMountPaths() async throws {
    let mountPath = "/tmp/metalpilot-canonical-\(UUID().uuidString)"
    ...
    func run(_ executable: String, arguments: [String]) async throws -> CommandResult {
        ...
        if executable == failingExecutable,
           failingArgumentsContain == nil || arguments.contains(where: { $0.contains(failingArgumentsContain!) }) {
            throw MetalPilotError.commandFailed("fixture launch failure")
        }
        return CommandResult(exitCode: 0, standardOutput: Data(), standardError: Data())
    }
}
...
@Test func importedHUDPresetRejectsCorruptAndUnsupportedValues() throws {
    ...
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: corrupt.path) }

    // Missing file
    let missing = root.appendingPathComponent("missing.plist")
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: missing.path) }
    ...
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: arrayPlist.path) }
    ...
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: badValue.path) }
}
...
@Test func managedHUDPresetStoreLeavesNoCopyForInvalidFile() async throws {
    ...
    let store = ManagedHUDPresetStore(directoryURL: managedDirectory)
    await #expect(throws: MetalPilotError.self) {
...
@Test func importedHUDPresetRejectsDirectoryEmptyFileAndUnsupportedTypes() throws {
    ...
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: directory.path) }

    // An empty file is not a valid property list.
    let empty = root.appendingPathComponent("empty.plist")
    try Data().write(to: empty)
    #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: empty.path) }
    ...
    for (name, value) in unsupportedValues {
        ...
        #expect(throws: MetalPilotError.self) { _ = try GamingService.metalHUDEnvironment(fromPresetAt: preset.path) }
    }
...
@Test func iosDeviceParsingRejectsMalformedJSON() {
    #expect(throws: MetalPilotError.self) { try GamingService.parseIOSDevices("not json at all") }
    #expect(throws: MetalPilotError.self) { try GamingService.parseIOSApps("<html>error</html>") }
}
...
@Test func iosLaunchArgumentsRejectNULNewlineAndExcessCount() throws {
    ...
    #expect(throws: MetalPilotError.self) { try GamingService.validateIOSLaunchArguments(["bad\0arg"]) }
    #expect(throws: MetalPilotError.self) { try GamingService.validateIOSLaunchArguments(["bad\narg"]) }
    #expect(throws: MetalPilotError.self) { try GamingService.validateIOSLaunchArguments(["bad\rarg"]) }
    ...
    #expect(throws: MetalPilotError.self) { try GamingService.validateIOSLaunchArguments(overCap) }
}
...
actor MissingXcodeCommandRunner: CommandRunning {
    func run(_ executable: String, arguments: [String]) async throws -> CommandResult {
        throw MetalPilotError.commandFailed("xcrun: error: unable to find utility \"devicectl\", not a developer tool or in PATH")
    }
}
